...
const char * SERVER_MAIN_PATH = "/home/adam/Documents/Programming/CourseWorkFTPServer/server_storage"; //68
...
const int AMMOUNT_OF_COMMANDS = 13; //from 17
const size_t BYTES_TO_SEND_AND_TO_GET = 36 * 1024;
...
//GETTING COMMAND NAME FROM LINE THAT WE'VE READ
...
//GET PARAMETER THAT STANDS ON HIS "parameter_number" PLACE
...
char * cwd_command_process(char * current_directory, char * buffer, int * cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 1){
        write(socket, MESSAGE_501, 44);
    ...
    if(parameter[0] == '.'){
        ...
        char * full_path = (char *)malloc(((*cur_dir_size) + path_size + 68)*sizeof(char));
        snprintf(full_path, (*cur_dir_size) + path_size + 68, "%s%s", SERVER_MAIN_PATH, final_path);
        printf("%s\n", full_path);
        ...
        char * full_path = (char *)malloc((strlen(parameter) + 68)*sizeof(char));
        snprintf(full_path, strlen(parameter) + 68, "%s%s", SERVER_MAIN_PATH, parameter);
        printf("%s\n", full_path);
        ...
        char * full_path = (char *)malloc(((*cur_dir_size) + strlen(parameter) + 1 + 68)*sizeof(char));
        snprintf(full_path, (*cur_dir_size) + strlen(parameter) + 1 + 68, "%s%s", SERVER_MAIN_PATH, final_path);
        printf("%s\n", full_path);
...
/// @param cur_dir_size - размер строки "current_directory"
/// @param socket - дескриптор сокета
void list_command_process(char * current_directory, char * buffer, int cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 0){
        write(socket, MESSAGE_501, 44);
        return;
    }
    ...
    char * full_cur_dir = (char *)malloc((cur_dir_size + 68)*sizeof(char));
    snprintf(full_cur_dir, cur_dir_size + 68, "%s%s", SERVER_MAIN_PATH, current_directory);
...
/// @param cur_dir_size - размер строки "current_directory"
/// @param socket - дескриптор сокета
void dele_command_process(char * current_directory, char * buffer, int cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 1){
        write(socket, MESSAGE_501, 44);
        return;
    }
    ...
    char * path_to_delete = (char *)malloc((68 + cur_dir_size + param_size) * sizeof(char));
    snprintf(path_to_delete, 68 + cur_dir_size + param_size, "%s%s%s", SERVER_MAIN_PATH, current_directory, parameter);
...
char * rnfr_command_process(char * current_directory, char * buffer, int cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 1){
        write(socket, MESSAGE_501, 44);
        return NULL;
    }
    ...
    int param_size = 0;
    for(param_size; parameter[param_size] != '\0'; param_size++);
    char * path_to_check = (char *)malloc((68 + cur_dir_size + param_size) * sizeof(char));
    snprintf(path_to_check, 68 + cur_dir_size + param_size, "%s%s%s", SERVER_MAIN_PATH, current_directory, parameter);
    fprintf(stderr, "%s\n", path_to_check);
...
/// @param cur_dir_size - размер строки "current_directory"
/// @param socket - дескриптор сокета
int rnto_command_process(char * path_to_rename, char * current_directory, char * buffer, int cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 1){
        write(socket, MESSAGE_501, 44);
        return -1;
    }
    ...
    int param_size = 0;
    for(param_size; parameter[param_size] != '\0'; param_size++);
    char * new_name = (char *)malloc((68 + cur_dir_size + param_size) * sizeof(char));
    snprintf(new_name, 68 + cur_dir_size + param_size, "%s%s%s", SERVER_MAIN_PATH, current_directory, parameter);
...
/// @param cur_dir_size - размер строки "current_directory"
/// @param socket - дескриптор сокета
void retr_command_process(char * current_directory, char * buffer, int cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 1){
        write(socket, MESSAGE_501, 44);
        return;
    }
    ...
    int param_size = 0;
    for(param_size; parameter[param_size] != '\0'; param_size++);
    char * file_name = (char *)malloc((68 + cur_dir_size + param_size) * sizeof(char));
    snprintf(file_name, 68 + cur_dir_size + param_size, "%s%s%s", SERVER_MAIN_PATH, current_directory, parameter);
...
/// @param cur_dir_size - размер строки "current_directory"
/// @param socket - дескриптор сокета
void stor_command_process(char * current_directory, char * buffer, int cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 1){
        write(socket, MESSAGE_501, 44);
        return;
    }
    ...
    char * file_path = (char *)malloc((68 + cur_dir_size + filename_size) * sizeof(char));
    snprintf(file_path, 68 + cur_dir_size + filename_size, "%s%s%s", SERVER_MAIN_PATH, current_directory, filename);
    write(socket, MESSAGE_150, 52);
    read(socket, buffer, 1);
...
/// @param cur_dir_size - размер строки "current_directory"
/// @param socket - дескриптор сокета
void zip_command_process(char * current_directory, char * buffer, int cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 1){
        write(socket, MESSAGE_501, 44);
        return;
    }
    ...
    char * file_path = (char *)malloc((68 + cur_dir_size + param_size)*sizeof(char));
    snprintf(file_path, 68 + cur_dir_size + param_size, "%s%s%s", SERVER_MAIN_PATH, current_directory, parameter);
    ...
    if(S_ISDIR(path_stat.st_mode)){
        ...
        param_size += 4;
        parameter = (char *)realloc(parameter, param_size*sizeof(char));
        parameter[param_size-4] = '.';
        parameter[param_size-3] = 'z';
        parameter[param_size-2] = 'i';
        parameter[param_size-1] = 'p';
        parameter[param_size] = '\0';

        fprintf(stderr, "%s\n", parameter);

        char * zip_path = (char *)malloc((68 + cur_dir_size + param_size)*sizeof(char));
        snprintf(zip_path, 68 + cur_dir_size + param_size, "%s%s%s", SERVER_MAIN_PATH, current_directory, parameter);
        fprintf(stderr, "%s\n", zip_path);

        int file_path_size = 0, zip_path_size = 0;
        for(file_path_size; file_path[file_path_size] != '\0'; file_path_size++);
        for(zip_path_size; zip_path[zip_path_size] != '\0'; zip_path_size++);

        final_command = (char *)malloc((9 + file_path_size + zip_path_size)*sizeof(char));
        snprintf(final_command, 19 + file_path_size + zip_path_size, "(cd %s && zip -r %s .)", file_path, zip_path); 
    }
    else{
        ...
        final_command = (char *)malloc((9 + file_path_size + zip_path_size)*sizeof(char));
        snprintf(final_command, 26 + file_path_size + param_size, "zip -j ./server_storage/%s %s", parameter, file_path); 
    }
...
/// @param cur_dir_size - размер строки "current_directory"
/// @param socket - дескриптор сокета
void uzip_command_process(char * current_directory, char * buffer, int cur_dir_size, int socket){
    if(get_ammount_of_parameters(buffer) != 1){
        write(socket, MESSAGE_501, 44);
        return;
    }
    ...
    char * command = (char *)malloc((68 + cur_dir_size + 68 + cur_dir_size + param_size + 7)*sizeof(char));
    snprintf(command, 68 + cur_dir_size + 68 + cur_dir_size + param_size + 11, "unzip %s -d %s%s", zip_path, SERVER_MAIN_PATH, current_directory);
...
//MAIN WORKING PROCESSS
void main_working_process(int socket){
    int working_stage = 3, cur_dir_size = 1;
    ...
    while(1){
        ...
        if(strcmp(command, "USER") == 0 && working_stage == 0){
            ...
            current_directory = cwd_command_process(current_directory, buffer, &cur_dir_size, socket);
        }
            ...
            list_command_process(current_directory, buffer, cur_dir_size, socket);
        }
        else if(strcmp(command, "DELE") == 0 && working_stage > 2){
            dele_command_process(current_directory, buffer, cur_dir_size, socket);
        }
        else if(strcmp(command, "RNFR") == 0 && working_stage > 2){
            char * path_to_rename;
            if((path_to_rename = rnfr_command_process(current_directory, buffer, cur_dir_size, socket)) != NULL){
                while(1){
                    for(int i = 0; buffer[i] != '\0'; i++){
                        buffer[i] = '\0';
                    }
                    ...
                    if(strcmp(command, "RNTO") == 0){
                        if(rnto_command_process(path_to_rename, current_directory, buffer, cur_dir_size, socket) == 0){
                            break;
                        }
                    }
            ...
            retr_command_process(current_directory, buffer, cur_dir_size, socket);
        }
        else if(strcmp(command, "STOR") == 0 && working_stage > 2){
            stor_command_process(current_directory, buffer, cur_dir_size, socket);
        }
        else if(strcmp(command, "ZIP") == 0 && working_stage > 2){
            zip_command_process(current_directory, buffer, cur_dir_size, socket);
        }
        else if(strcmp(command, "UZIP") == 0 && working_stage > 2){
            uzip_command_process(current_directory, buffer, cur_dir_size, socket);
        }
...
int main() {
    ...
    main_working_process(socket_data[1]);
