...
char * write_from_buffer(char * buffer){
    int string_size = 1;
...
int main() {
    ...
    while(1){
        ...
        if(strcmp(command, "QUIT") == 0 && strstr(buffer, "503 Bad sequence of commands") == 0){
        ...
        else if(strcmp(command, "RETR") == 0 && strstr(buffer, "150 File status okay; about to open data connection") != NULL){
            ...
            if (access(parameter, F_OK) == 0 && stat(parameter, &st) == 0 && S_ISREG(st.st_mode)) {
                ...
                read(sock, buffer, 1);
                unsigned char * data_buffer = (unsigned char *)malloc(BYTES_TO_SEND_AND_TO_GET*sizeof(unsigned char));
                FILE * file = fopen(parameter, "rb"); // или .png, .pdf
                if (file < 0) {
                    perror("Error opening file");
                    exit(1);
                }
